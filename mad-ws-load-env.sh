...
export DB_NAME=$(aws ssm get-parameters --names /cdk/rds/db --query 'Parameters[0].Value' --output text);
export SECRET_NAME=$(aws ssm get-parameters --names /cdk/rds/secret --query 'Parameters[0].Value' --output text);
